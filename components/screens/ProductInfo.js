...
import {
    ...
    Animated,
    ToastAndroid,
    TextInput, Keyboard, TouchableWithoutFeedback,Alert
} from 'react-native';
...
const ProductInfo = ({ route, navigation }) => {
    const { markProduct, userToken, removeMarkProduct, isMarkProduct,addRating } = useContext(AuthContext);
    ...
    const addNewRating = async () => {
        const res = await addRating(rateData, navigation)
        if (res) {
        ...
        return (
            <View
                ...
                {!item.isVideo ?
                    ...
                    /> :
                    <Video
                        source={{ uri: UPLOAD_VIDEOS_URL + (item.url ? item.url : 'ok_hdtjkn') + '.mov' }}
                        style={{
                            width: '100%',
                            height: '100%',
                            resizeMode: 'contain',
    ...
    return (
        <View
            ...
            <ScrollView>
                ...
                <View
                    ...
                    <View style={{
                        ...
                        <View
                            ...
                            <Image
                                ...
                                source={{ uri: UPLOAD_IMAGES_URL + (user?.avatarUrl ? user.avatarUrl : 'ok_b7poza') + '.png' }}
                            />
                    ...
                    <View style={{ alignItems: 'center' }}>
                        {userToken ?
                            <TouchableOpacity onPress={() => { setShowRating(true) }} style={{ paddingHorizontal:15,paddingVertical:10, borderWidth: 0.5, borderRadius: 10, borderColor: 'orange' }} >
                                <Text style={{ color: 'orange' }}>Đánh giá</Text>
                            </TouchableOpacity>
                            :
                            <TouchableOpacity onPress={() => { navigation.navigate('Login') }} style={{ padding: 5, borderWidth: 0.5, borderRadius: 10, borderColor: 'orange' }}>
                                <Text style={{ color: 'orange' }}>Đăng nhập để đánh giá</Text>
                            </TouchableOpacity>
                        }
                    </View>
            ...
            <Modal style={{ justifyContent: 'center', alignItems: 'center' }} isVisible={showRating} backdropOpacity={0.2} >
                ...
                <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
                    <View style={{ width: '100%', height: '70%', position: 'relative', padding: 10, justifyContent: 'center', alignItems: 'center', backgroundColor: 'white' }}>
                        ...
                        <Rating imageSize={28} startingValue={rateData.rating} onFinishRating={(e) => { handleOnChange('rating', e) }} />
                        <Text style={{ marginTop: 10, fontWeight: '600', fontSize: rateData.rating * 2 + 10 }}>{rateData.rating}</Text>

                        <View
                            style={{ width: '80%', height: 200, borderColor: '#D5D5D5', marginTop: 20, borderRadius: 10, borderWidth: 1, justifyContent: 'center', paddingVertical: 10, paddingHorizontal: 15 }}>
                            <TextInput
                                editable
                                multiline
                                numberOfLines={3}
                                ...
                                onChangeText={(e) => { handleOnChange('comment', e) }}
                            ></TextInput>
                        </View>
                        <TouchableOpacity
                        ...
                        <TouchableOpacity
                            ...
                            style={{backgroundColor:'orange',marginTop:20,paddingVertical:10,paddingHorizontal:15, borderRadius:10}}>
                            <Text style={{color:'white',fontWeight:'700'}} >Đánh giá</Text>
                        </TouchableOpacity>
                    </View>
