...
//    }, []);
//};
    const addRating = async (data, navigation) => {
        let status = false;
        if (userToken) {
            const resp = await fetch(`${API_URL}/api/UserRating/add`, {
                method: "post",
                headers: {
                    'Accept': 'application/json',
                    'Content-type': 'application/json',
    ...
    return (
        <AuthContext.Provider value={{ login, register, logout, userToken, user, loginState, isMarkProduct, markProduct, removeMarkProduct, addRating, fomatTime }}>
            {children }
